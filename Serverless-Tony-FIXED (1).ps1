...
# ---------------------------------------------------------------
# STEP 1: USE EXISTING SESSION - NO LOGIN POPUP
# ---------------------------------------------------------------
Write-Host "[1/4] Checking Azure session..." -ForegroundColor Yellow

$ctx = Get-AzContext -ErrorAction SilentlyContinue

if ($ctx -and $ctx.Account -and $ctx.Tenant.Id -eq $TenantId) {
    Write-Host "  Already connected as: $($ctx.Account.Id)" -ForegroundColor Green
} elseif ($ctx -and $ctx.Account) {
    Write-Host "  Session found: $($ctx.Account.Id) - switching tenant..." -ForegroundColor Yellow
    try {
        Connect-AzAccount -TenantId $TenantId -AccountId $ctx.Account.Id -WarningAction SilentlyContinue | Out-Null
        $ctx = Get-AzContext
        Write-Host "  Connected as: $($ctx.Account.Id)" -ForegroundColor Green
    } catch {
        # Already connected, just proceed
        $ctx = Get-AzContext
        Write-Host "  Using session: $($ctx.Account.Id)" -ForegroundColor Green
    }
} else {
    Write-Host "  No active session found - please run Connect-AzAccount first" -ForegroundColor Red
    Write-Host "  Run: Connect-AzAccount -TenantId $TenantId" -ForegroundColor Yellow
