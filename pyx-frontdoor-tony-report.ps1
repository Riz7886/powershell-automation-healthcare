...
# Bulletproof integer extraction - handles any noise (PowerShell trace, stderr leak, etc.)
# Walks lines, returns first pure-numeric line as int, else 0.
function Get-AzInt {
    param($Raw)
...
foreach ($cp in $classicList) {
    ...
    # Routing rules count
    $ruleCount = Get-AzInt (az network front-door routing-rule list -g $ResourceGroup --front-door-name $cp --query "length([])" -o tsv 2>$null)

    # Backend pools count
    $bpCount = Get-AzInt (az network front-door backend-pool list -g $ResourceGroup --front-door-name $cp --query "length([])" -o tsv 2>$null)
...
foreach ($p in $cdnList) {
    ...
    $cdCount      = Get-AzInt (az afd custom-domain list -g $ResourceGroup --profile-name $p.Name --query "length([])" -o tsv 2>$null)
    $epCount      = Get-AzInt (az afd endpoint list -g $ResourceGroup --profile-name $p.Name --query "length([])" -o tsv 2>$null)
    $ogCount      = Get-AzInt (az afd origin-group list -g $ResourceGroup --profile-name $p.Name --query "length([])" -o tsv 2>$null)
    $secCount     = Get-AzInt (az afd security-policy list -g $ResourceGroup --profile-name $p.Name --query "length([])" -o tsv 2>$null)
    $ruleSetCount = Get-AzInt (az afd rule-set list -g $ResourceGroup --profile-name $p.Name --query "length([])" -o tsv 2>$null)
