...
foreach ($p in $plan) {
    ...
    if ($p.WafPolicies.Count -gt 0) {
        Log "Step 2 - Pre-creating new Standard-tier WAF policies (Microsoft requires pre-creation for programmatic migration)..."
        foreach ($w in $p.WafPolicies) {
            try {
                ...
                if ($existingNew) {
                    Log "  New WAF '$newWafName' already exists - reusing" "OK"
                    $newWafId = $existingNew.Id
                } else {
                    Log "  Creating new WAF '$newWafName' with SKU Standard_AzureFrontDoor..."
