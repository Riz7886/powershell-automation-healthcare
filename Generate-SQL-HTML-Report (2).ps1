...
$totalSavings = 0
foreach ($row in $data) {
    $cost = [decimal]($row.CurrentMonthlyCost -replace '[^0-9.-]','')
    $savings = [decimal]($row.PotentialSavings -replace '[^0-9.-]','')
...
Write-Host "DEBUG - Calculations:" -ForegroundColor Yellow
Write-Host "  Total Databases: $totalDatabases" -ForegroundColor Cyan
Write-Host "  Underutilized: $underutilized" -ForegroundColor Cyan
Write-Host "  Current Monthly Cost: `$$($totalCurrentCost.ToString('N2'))" -ForegroundColor Cyan
Write-Host "  Total Monthly Savings: `$$($totalSavings.ToString('N2'))" -ForegroundColor Cyan
Write-Host "  After Optimization: `$$($afterOptimization.ToString('N2'))" -ForegroundColor Cyan
Write-Host "  Annual Savings: `$$($annualSavings.ToString('N2'))" -ForegroundColor Green

$topSavings = $data | ForEach-Object {
    $_ | Add-Member -NotePropertyName "SavingsNum" -NotePropertyValue ([Math]::Abs([decimal]($_.PotentialSavings -replace '[^0-9.-]',''))) -PassThru
} | Sort-Object SavingsNum -Descending | Select-Object -First 15
    ...
    <div class="container">
        ...
        <div class="chart-container">
            ...
            <div class="chart-box">
                ...
                <p style="margin: 15px 0;"><strong>After Optimization:</strong> <span class="savings-amount">`$$('{0:N2}' -f $afterOptimization)/month</span></p>
...
foreach ($db in $topSavings) {
    ...
    $html += @"
                <tr>
                    ...
                    <td class="savings-amount">`$$('{0:N2}' -f $db.SavingsNum)/mo</td>
                </tr>
"@
}
        ...
        <div class="section">
            ...
            <table>
                ...
                <tr>
                    ...
                    <td class="savings-amount">`$$('{0:N2}' -f (($data | Where-Object { $_.RecommendedTier -eq "Basic" } | ForEach-Object { [Math]::Abs([decimal]($_.PotentialSavings -replace '[^0-9.-]','')) } | Measure-Object -Sum).Sum))/mo</td>
                </tr>
                <tr>
                    ...
                    <td class="savings-amount">`$$('{0:N2}' -f (($data | Where-Object { $_.RecommendedTier -like "S*" -and $_.RecommendedTier -ne $_.ServiceObjective } | ForEach-Object { [Math]::Abs([decimal]($_.PotentialSavings -replace '[^0-9.-]','')) } | Measure-Object -Sum).Sum))/mo</td>
                </tr>
                <tr>
                    ...
                    <td class="savings-amount">`$$('{0:N2}' -f (($data | Where-Object { $_.RecommendedTier -like "P*" -and $_.RecommendedTier -ne $_.ServiceObjective } | ForEach-Object { [Math]::Abs([decimal]($_.PotentialSavings -replace '[^0-9.-]','')) } | Measure-Object -Sum).Sum))/mo</td>
                </tr>
            </table>
        </div>
    </div>
