...
$logPath  = Join-Path $reportDir "migration-v12-$stamp.log"
$jsonPath = Join-Path $reportDir "results-v12-$stamp.json"
$htmlPath = Join-Path $reportDir "PYX-AFD-Migration-Report-v12-$stamp.html"
...
foreach ($info in $plan) {
    ...
    if ($info.Action -eq "MIGRATE") {
        ...
        Info "Polling Azure for new profile to appear (up to 20 minutes)..."
        $waitMaxSec = 1200
        $pollInterval = 20
        $elapsed = 0
        $prepareReady = $false
        while ($elapsed -lt $waitMaxSec) {
            ...
            if ($checkProfile) {
                $state = $checkProfile.properties.provisioningState
                Info "  [${elapsed}s] $($info.Standard) exists, provisioningState=$state"
                if ($state -in @("Succeeded","Migrated")) {
                    $prepareReady = $true
                    Ok "Prepare complete after ${elapsed}s. Ready to commit."
                    break
                }
            } else {
                Info "  [${elapsed}s] $($info.Standard) not visible yet..."
            }
        }
        if (-not $prepareReady) {
            $info.Status = "prepare-timeout"; $info.Error = "Prepare did not complete within ${waitMaxSec}s"
            Err "Prepare did NOT complete within ${waitMaxSec}s. Skipping commit."
            continue
        }
    }
