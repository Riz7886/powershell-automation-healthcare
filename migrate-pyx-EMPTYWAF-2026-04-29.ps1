...
if ($PSVersionTable.PSVersion.Major -lt 7) {
    Write-Host ""
    Write-Host "ERROR: This script requires PowerShell 7.2 or newer." -ForegroundColor Red
    Write-Host "Az.Cdn migration cmdlets are AutoRest-based and misbehave in Windows PowerShell 5.1." -ForegroundColor Red
    Write-Host "Current version: $($PSVersionTable.PSVersion)" -ForegroundColor Red
    Write-Host "Install PowerShell 7 from: https://aka.ms/powershell" -ForegroundColor Yellow
    Write-Host "Then close ISE and run this from 'pwsh' (PowerShell 7 console)." -ForegroundColor Yellow
    exit 20
}
...
foreach ($cp in $ProfileMap.Keys) {
    ...
    if ($found.Type -eq "AFD") {
        try {
            $afdProf = Get-AzFrontDoor -ResourceGroupName $found.Rg -Name $cp -ErrorAction SilentlyContinue
            $linkedWafIds = @{}

            if ($afdProf -and $afdProf.FrontendEndpoints) {
                foreach ($fe in $afdProf.FrontendEndpoints) {
                    if ($fe.WebApplicationFirewallPolicyLink -and $fe.WebApplicationFirewallPolicyLink.Id) {
                        $linkedWafIds[$fe.WebApplicationFirewallPolicyLink.Id] = "frontendEndpoint"
                    }
                }
            }

            if ($afdProf -and $afdProf.WebApplicationFirewallPolicyLink) {
                foreach ($lnk in @($afdProf.WebApplicationFirewallPolicyLink)) {
                    if ($lnk -and $lnk.Id) { $linkedWafIds[$lnk.Id] = "topLevel" }
                }
            }

            $allWafsInRg = Get-AzFrontDoorWafPolicy -ResourceGroupName $found.Rg -ErrorAction SilentlyContinue
            foreach ($w in $allWafsInRg) {
                if ($w.Sku -ne "Classic_AzureFrontDoor") { continue }
                $linksToCheck = @()
                if ($w.FrontendEndpointLinks) { $linksToCheck += @($w.FrontendEndpointLinks) }
                if ($w.FrontendEndpointLink)  { $linksToCheck += @($w.FrontendEndpointLink) }
                foreach ($felink in $linksToCheck) {
                    $felinkId = if ($felink.Id) { $felink.Id } else { "$felink" }
                    if ($felinkId -match "/frontDoors/$([regex]::Escape($cp))/frontendEndpoints/") {
                        $linkedWafIds[$w.Id] = "reverseLookup"
                        break
                    }
                }
            }

            foreach ($wid in $linkedWafIds.Keys) {
                $wafName = $wid.Split('/')[-1]
                $wafRg   = $wid.Split('/')[4]
                $wafObj  = Get-AzFrontDoorWafPolicy -ResourceGroupName $wafRg -Name $wafName -ErrorAction SilentlyContinue
                if ($wafObj -and $wafObj.Sku -eq "Classic_AzureFrontDoor") {
                    $wafPolicies += $wafObj
                    Log "  Linked WAF: $wafName  (source: $($linkedWafIds[$wid]))" "OK"
                }
            }
        } catch {
    ...
    Log "$($wafPolicies.Count) Classic WAF policy(ies) attached to profile $cp"
...
foreach ($p in $plan) {
    ...
    $skuToUse = if ($testResult.DefaultSku) { $testResult.DefaultSku } else { $Sku }
    Log "Using SKU: $skuToUse (from Test result)"

    $wafMappings = @()
    if ($p.WafPolicies.Count -gt 0) {
        Log "Step 2 - Pre-creating new $skuToUse-tier WAF policies (Microsoft requires pre-creation + matching tier)..."
        foreach ($w in $p.WafPolicies) {
            try {
                $newWafName = "$($w.Name)" + $(if ($skuToUse -like "Premium*") { "Prem" } else { "Std" })
                ...
                if ($existingNew -and $existingNew.Sku -ne $skuToUse) {
                    Log "  WAF '$newWafName' exists but with wrong SKU ($($existingNew.Sku) vs $skuToUse) - deleting to recreate" "WARN"
                    Remove-AzFrontDoorWafPolicy -ResourceGroupName $p.ResourceGroup -Name $newWafName -ErrorAction SilentlyContinue | Out-Null
                    $existingNew = $null
                }
                if ($existingNew) {
                    Log "  New WAF '$newWafName' already exists with matching SKU - reusing" "OK"
                    $newWafId = $existingNew.Id
                } else {
                    Log "  Creating new WAF '$newWafName' with SKU $skuToUse..."
                    $newWaf = New-AzFrontDoorWafPolicy `
                        -Name $newWafName `
                ...
                Log "  WAF pre-create / mapping failed for $($w.Name): $($_.Exception.Message)" "WARN"
            }
        }
    }

    Log "Step 3 - Start-AzFrontDoorCdnProfilePrepareMigration with SKU $skuToUse..."
    try {
        $prepParams = @{
            ResourceGroupName                       = $p.ResourceGroup
        ...
        Log "Prepare migration succeeded - new $skuToUse profile created (classic still serving)" "OK"
    } catch {
        $errText = $_.Exception.Message
