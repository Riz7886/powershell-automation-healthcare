...
try {
    ...
    Write-Host "  Hosts skipped: $($_.Exception.Message)" -ForegroundColor DarkYellow
}
...
$staleHostsHtml = if ($staleHosts.Count -eq 0) { "<p class='ok'>All hosts reporting within $StaleMinutes min (or hosts API was skipped).</p>" } else {
...
$pausedSynthHtml = if ($pausedSynth.Count -eq 0) { "<p class='ok'>No paused synthetic tests (or synthetics API was skipped).</p>" } else {
...
.kpi.bad .n { color:#dc2626; } .kpi.warn .n { color:#d97706; } .kpi.ok .n { color:#059669; }
...
<div class="kpis">
  ...
  <div class="kpi"><div class="n">$($staleHosts.Count)</div><div class="l">Stale Hosts</div></div>
  <div class="kpi"><div class="n">$($pausedSynth.Count)</div><div class="l">Paused Synthetics</div></div>
  ...
  <div class="kpi"><div class="n">$totalServices</div><div class="l">APM Services</div></div>
</div>

<section>
  <h2>Monitors in No-Data State <span class="count">($($noDataMonitors.Count))</span></h2>
  <p style="font-size:12px;color:#6b7280;margin-bottom:14px;">Monitors that ran but received zero data points during their evaluation window. The Query column shows the metric expression each monitor depends on - if the query references a metric that no longer publishes, that's the root cause.</p>
  $noDataMonitorsHtml
</section>
...
$summary = [pscustomobject]@{
    ...
    okMonitors        = $okMonitors.Count
    alertMonitors     = $alertMonitors.Count
    warnMonitors      = $warnMonitors.Count
    mutedMonitors     = $mutedMonitors.Count
    dbxTotal          = $dbxMonitors.Count
    dbxOk             = $dbxOk.Count
    dbxNoData         = $dbxNoData.Count
