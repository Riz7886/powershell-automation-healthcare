...
try {
    ...
    if ($existingSp -and $existingSp.Count -gt 0) {
        ...
        Write-Log "  SP exists: $spObjectId" "Green"
    }
}
catch {}
...
if (-not $spObjectId) {
    try {
        $newSp = az ad sp create --id $appId -o json 2>&1 | ConvertFrom-Json
        $spObjectId = $newSp.id
        Write-Log "  Created SP: $spObjectId" "Green"
    }
    catch {
        Write-Log "  FAILED to create SP: $_" "Red"
        $allResults.errors += "SP creation failed: $_"
    }
}
...
try {
    $credResult = az ad app credential reset --id $appId --display-name $secretName --end-date $secretExpiry --append -o json 2>&1 | ConvertFrom-Json
...
$hasAzModule = Get-Module -ListAvailable -Name Az.Sql -ErrorAction SilentlyContinue
if (-not $hasAzModule) {
    Write-Log "  Installing Az.Sql module..." "Yellow"
    try { Install-Module -Name Az.Sql -Scope CurrentUser -Force -AllowClobber } catch {}
    try { Install-Module -Name Az.Monitor -Scope CurrentUser -Force -AllowClobber } catch {}
}

try {
    $azCtx = Get-AzContext
    if (-not $azCtx) {
        Write-Log "  Connecting Az module..." "Yellow"
        Connect-AzAccount | Out-Null
    }
}
catch {
    Write-Log "  Az module connection issue, using CLI fallback" "Yellow"
}
...
Write-Log "[2.1] Scanning all subscriptions..." "Yellow"
Write-Host ""
...
foreach ($sub in $allSubs) {
    ...
    az account set --subscription $sub.id 2>$null
    try { Set-AzContext -SubscriptionId $sub.id -ErrorAction SilentlyContinue | Out-Null } catch {}

    $servers = az sql server list --subscription $sub.id -o json 2>$null | ConvertFrom-Json
    ...
    foreach ($srv in $servers) {
        ...
        $databases = az sql db list --server $serverName --resource-group $rg --subscription $sub.id -o json 2>$null | ConvertFrom-Json
        ...
        foreach ($db in $databases) {
            ...
            $currentSizeGB = 0
            try {
                $usageJson = az sql db list-usages --server $serverName --name $dbName --resource-group $rg --subscription $sub.id -o json 2>$null | ConvertFrom-Json
            ...
            $maxDtu = 0
            try {
                $dtuMetrics = az monitor metrics list --resource $db.id --metric "dtu_consumption_percent" --start-time $startTime --end-time $endTime --interval PT1H --aggregation Average -o json 2>$null | ConvertFrom-Json
            ...
            $totalConnections = 0
            try {
                $connMetrics = az monitor metrics list --resource $db.id --metric "connection_successful" --start-time $startTime --end-time $endTime --interval P1D --aggregation Total -o json 2>$null | ConvertFrom-Json
