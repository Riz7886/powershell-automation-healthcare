...
$WorkspaceUrl = "https://adb-3248848193480666.6.azuredatabricks.net"
$ServicePrincipalDisplayName = "databricks-dev-svc"
...
$BackupFolder = Join-Path $OutputFolder "backups-dev-full"
...
$reportPath = Join-Path $OutputFolder "Databricks-DEV-Full-$timestamp.html"
$spOutputPath = Join-Path $OutputFolder "Databricks-DEV-ServicePrincipal-$timestamp.txt"

Write-Host ""
Write-Host "================================================================" -ForegroundColor Cyan
Write-Host "  DATABRICKS DEV FULL MIGRATION - $(if ($Apply) { 'APPLY' } elseif ($Rollback) { 'ROLLBACK' } else { 'DRY RUN' })" -ForegroundColor Cyan
Write-Host "================================================================" -ForegroundColor Cyan
...
if ($existing) {
    ...
    $secretBody = @"
=========================================================
DATABRICKS DEV SERVICE PRINCIPAL CREDENTIALS - SAVE NOW
=========================================================
...
if ($Apply -and ($flipResults.Count -gt 0 -or $aclResults.Count -gt 0)) {
    ...
    foreach ($f in $flipResults) {
        Write-Host "  .\Databricks-DEV-Full.ps1 -Rollback `"$($f.Backup)`" -Apply" -ForegroundColor Yellow
    }
    foreach ($a in $aclResults) {
        Write-Host "  .\Databricks-DEV-Full.ps1 -Rollback `"$($a.Backup)`" -Apply" -ForegroundColor Yellow
    }
}
...
$html = @"
<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"><title>Databricks DEV Full</title>
<style>
...
<h1>Databricks DEV Full Migration</h1>
