...
Write-Host "  FIX pyx-warehouse-prod - Serverless to PRO" -ForegroundColor Cyan
Write-Host "========================================================" -ForegroundColor Cyan
Write-Host ""

$wsHost = "https://adb-2756318932417370.6.azuredatabricks.net"
Write-Host "  Workspace: pyx-warehouse-prod" -ForegroundColor Green
Write-Host "  URL: $wsHost" -ForegroundColor Green
Write-Host ""

Write-Host "  Generate a PAT from THIS workspace:" -ForegroundColor Yellow
Write-Host "  1. Open $wsHost in your browser" -ForegroundColor Yellow
Write-Host "  2. Click user icon (top right) > Settings > Developer" -ForegroundColor Yellow
Write-Host "  3. Click Manage next to Access tokens > Generate new token" -ForegroundColor Yellow
Write-Host "  4. Paste it below" -ForegroundColor Yellow
Write-Host ""
...
# ---------------------------------------------------------------
Write-Host ""
Write-Host "[1/3] Listing SQL warehouses..." -ForegroundColor Yellow
...
try {
    ...
    Write-Host "  - You generated the PAT from pyx-warehouse-prod (not pyxlake-databricks)"
    Write-Host "  - Your account has admin access on this workspace"
    exit 1
}
...
if ($brokenWarehouses.Count -eq 0) {
    Write-Host "  All warehouses are already Classic or Pro. Nothing to fix." -ForegroundColor Green
    exit 0
}
...
Write-Host "  Only changing type. Everything else stays the same." -ForegroundColor Yellow
Write-Host ""
...
foreach ($wh in $brokenWarehouses) {
    ...
    try {
        ...
        Write-Host "    ID: $whId"
        Write-Host "    Changed: Serverless -> PRO"
        Write-Host "    Name: unchanged"
        Write-Host "    Size: unchanged ($($currentWh.cluster_size))"
        Write-Host "    Auto-stop: unchanged"
        Write-Host "    Clusters: unchanged"
        $fixed++
    }
    catch {
...
try {
    ...
    Write-Host ""
    if ($stillBroken -eq 0) {
        Write-Host "  ALL WAREHOUSES VERIFIED - No serverless remaining." -ForegroundColor Green
    }
    else {
...
Write-Host "========================================================" -ForegroundColor Cyan
Write-Host "  Workspace: pyx-warehouse-prod"
Write-Host "  Fixed: $fixed" -ForegroundColor Green
Write-Host "  Failed: $failed" -ForegroundColor $(if ($failed -gt 0) { "Red" } else { "Green" })
Write-Host ""

if ($fixed -gt 0 -and $failed -eq 0) {
    Write-Host "  SUCCESS - Go start the warehouses in Databricks now." -ForegroundColor Green
    Write-Host "  sql-warehouse and pyx-warehouse are now PRO type." -ForegroundColor Green
}
Write-Host ""
