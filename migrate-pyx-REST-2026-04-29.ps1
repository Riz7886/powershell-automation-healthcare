...
foreach ($p in $plan) {
    ...
    if (-not $p.StandardExists) {
        $migUrl = "https://management.azure.com/subscriptions/$($p.SubscriptionId)/resourceGroups/$($p.ResourceGroup)/providers/Microsoft.Cdn/profiles/$($p.Standard)/migrate?api-version=$ApiVersion"
        $migBody = @{
            sku = @{ name = $Sku }
