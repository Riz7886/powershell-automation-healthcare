    ...
    if ($found.Type -eq "AFD") {
        try {
            ...
            if ($afdProf -and $afdProf.FrontendEndpoints) {
                foreach ($fe in $afdProf.FrontendEndpoints) {
                    if ($fe.WebApplicationFirewallPolicyLink -and $fe.WebApplicationFirewallPolicyLink.Id) {
                        $wid = $fe.WebApplicationFirewallPolicyLink.Id
                        if (-not $linkedWafIds.ContainsKey($wid)) { $linkedWafIds[$wid] = $true }
                    }
                }
            }
            foreach ($wid in $linkedWafIds.Keys) {
                ...
                if ($wafObj -and $wafObj.Sku -eq "Classic_AzureFrontDoor") { $wafPolicies += $wafObj }
            }
        } catch {}
    }
    Log "$($wafPolicies.Count) Classic WAF policy(ies) linked to profile $cp"
