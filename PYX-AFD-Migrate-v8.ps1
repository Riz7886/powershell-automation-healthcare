...
function Get-CdnProfileSku {
    param([string]$Name, [string]$RG)
    $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "SilentlyContinue"
    try {
        $r = & az cdn profile show --profile-name $Name --resource-group $RG -o tsv --query "sku.name" --only-show-errors 2>$null
        if ([string]::IsNullOrWhiteSpace($r)) { return "" }
        return $r.ToString().Trim()
    } catch { return "" }
    finally { $ErrorActionPreference = $prevEAP }
...
$logPath  = Join-Path $reportDir "migration-v8-$stamp.log"
$jsonPath = Join-Path $reportDir "results-v8-$stamp.json"
$htmlPath = Join-Path $reportDir "PYX-AFD-Migration-Report-v8-$stamp.html"
Start-Transcript -Path $logPath -Force | Out-Null

Say "PYX AFD/CDN Classic-to-Standard Migration v8"
...
<p>Classic-to-Standard migration via Azure CLI (v8). Verification queries each new profile in Azure to confirm it exists on Standard_AzureFrontDoor SKU with provisioningState=Succeeded.</p>
...
<p class="meta">Generated $reportTimestamp | Script: PYX-AFD-Migrate-v8.ps1</p>
</body></html>
"@
