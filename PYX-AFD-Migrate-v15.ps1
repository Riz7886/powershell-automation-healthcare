...
$logPath  = Join-Path $reportDir "migration-v15-$stamp.log"
$jsonPath = Join-Path $reportDir "results-v15-$stamp.json"
$htmlPath = Join-Path $reportDir "PYX-AFD-Migration-Report-v15-$stamp.html"
Start-Transcript -Path $logPath -Force | Out-Null

Say "PYX AFD/CDN Migration v15 (REST + async polling + real error visibility)"
...
function Get-LinkedWafPolicies {
    param([string]$ClassicAfdId)
    $apiVer = "2020-05-01"
    $url = "https://management.azure.com${ClassicAfdId}?api-version=$apiVer"
    $r = Invoke-AzureRest -Method "GET" -Url $url
    if (-not $r.Ok) { return @() }
    try {
        $afd = $r.Body | ConvertFrom-Json
        $wafIds = @()
        if ($afd.properties.frontendEndpoints) {
            foreach ($fe in $afd.properties.frontendEndpoints) {
                if ($fe.properties.webApplicationFirewallPolicyLink -and $fe.properties.webApplicationFirewallPolicyLink.id) {
                    $wafIds += $fe.properties.webApplicationFirewallPolicyLink.id
                }
            }
        }
        return ($wafIds | Sort-Object -Unique)
    } catch { return @() }
}

function Ensure-StandardWafPolicy {
    param([string]$ClassicWafId)
    $parts = $ClassicWafId -split "/"
    $wafSubId = $parts[2]
    $wafRg    = $parts[4]
    $wafName  = $parts[-1]
    $newName  = "${wafName}Std"

    $newId = "/subscriptions/$wafSubId/resourceGroups/$wafRg/providers/Microsoft.Network/FrontDoorWebApplicationFirewallPolicies/$newName"
    $checkUrl = "https://management.azure.com${newId}?api-version=2024-02-01"
    $check = Invoke-AzureRest -Method "GET" -Url $checkUrl
    if ($check.Ok) {
        Info "  Standard WAF $newName already exists"
        return $newId
    }

    $classicUrl = "https://management.azure.com${ClassicWafId}?api-version=2024-02-01"
    $classicResp = Invoke-AzureRest -Method "GET" -Url $classicUrl
    if (-not $classicResp.Ok) {
        Warn "  Could not read classic WAF $wafName (will create empty Standard WAF)"
        $customRules = @{ rules = @() }
        $policySettings = @{ enabledState = "Enabled"; mode = "Detection" }
    } else {
        $classic = $classicResp.Body | ConvertFrom-Json
        $customRules = if ($classic.properties.customRules) { $classic.properties.customRules } else { @{ rules = @() } }
        $policySettings = if ($classic.properties.policySettings) { $classic.properties.policySettings } else { @{ enabledState = "Enabled"; mode = "Detection" } }
    }
    ...
    $putUrl = "https://management.azure.com${newId}?api-version=2024-02-01"
    Info "  Creating new Standard WAF: $newName"
    $putResp = Invoke-AzureRest -Method "PUT" -Url $putUrl -Body $newBody
    if (-not $putResp.Ok -and $putResp.StatusCode -notin @(200,201,202)) {
        Warn "  WAF create returned status $($putResp.StatusCode): $($putResp.Body)"
        return $null
    }
    Ok "  Created Standard WAF: $newName"
    return $newId
}
...
foreach ($p in $profiles) {
    ...
    $info.WafMappings = @()
    if ($p.Type -eq "AFD" -and $info.Action -eq "MIGRATE" -and $info.ClassicId) {
        $linkedWafs = Get-LinkedWafPolicies -ClassicAfdId $info.ClassicId
        Info "  Linked WAF policies: $($linkedWafs.Count)"
        foreach ($wId in $linkedWafs) {
            Info "    - $wId"
        }
        $info.LinkedWafIds = $linkedWafs
    }
...
Say "Phase 1.6 - Pre-create Standard-tier WAF policies for each linked classic WAF"
foreach ($info in $plan) {
    if ($info.Type -ne "AFD" -or $info.Action -ne "MIGRATE") { continue }
    if (-not $info.LinkedWafIds -or $info.LinkedWafIds.Count -eq 0) { continue }
    Info "Profile: $($info.Classic)"
    foreach ($oldWafId in $info.LinkedWafIds) {
        $newWafId = Ensure-StandardWafPolicy -ClassicWafId $oldWafId
        if ($newWafId) {
            $info.WafMappings += @{
                migratedFrom = @{ id = $oldWafId }
                migratedTo   = @{ id = $newWafId }
            }
        }
    }
}
...
foreach ($info in $plan) {
    ...
    if ($info.Action -eq "MIGRATE") {
        if ($info.Type -eq "AFD") {
            ...
            if ($info.WafMappings -and $info.WafMappings.Count -gt 0) {
                $bodyObj["migrationWebApplicationFirewallMappings"] = $info.WafMappings
                Info "Including $($info.WafMappings.Count) WAF mapping(s) in migrate body"
            }
...
$html = @"
<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>PYX Front Door Migration Report v15</title>
<style>
...
<p class="meta">Generated $reportTimestamp | Script: PYX-AFD-Migrate-v15.ps1 | API: Microsoft.Cdn 2025-04-15</p>
</body></html>
"@
