...
function Test-ClassicAfdExists {
    param([string]$Name, [string]$RG)
    $url = "https://management.azure.com/subscriptions/$Subscription/resourceGroups/$RG/providers/Microsoft.Network/frontdoors/${Name}?api-version=2024-02-01"
    $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "SilentlyContinue"
    try {
        $r = & az rest --method get --url $url --only-show-errors 2>$null
        if ([string]::IsNullOrWhiteSpace($r)) { return $null }
...
foreach ($p in $profiles) {
    ...
    if ($existing -and $existing.sku -and $existing.sku.name -in @("Standard_AzureFrontDoor","Premium_AzureFrontDoor") -and $existing.properties.provisioningState -eq "Succeeded") {
        ...
        if ($p.Type -eq "AFD") {
            ...
            $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "SilentlyContinue"
            try {
                $cdnRaw = & az rest --method get --url $url --only-show-errors 2>$null
                if (-not [string]::IsNullOrWhiteSpace($cdnRaw)) {
                    $cdn = $cdnRaw | ConvertFrom-Json
                    $info.ClassicId = $cdn.id
                    $info.Action = "MIGRATE"
                } else {
                    $info.Action = "NO-CLASSIC"
                }
            } catch { $info.Action = "NO-CLASSIC" }
            finally { $ErrorActionPreference = $prevEAP }
        }
    }
...
foreach ($info in $plan) {
    ...
    if ($info.Action -eq "MIGRATE") {
        ...
        Info "Step 1/2: POST .../Microsoft.Cdn/migrate"
        ...
        try {
            ...
            $exit = $LASTEXITCODE
            if ($exit -ne 0) { throw "az rest exit=$exit. Output: $($resp | Out-String)" }
        ...
        Info "Waiting 30 seconds for prepare to complete..."
        Start-Sleep -Seconds 30

        $checkProfile = Test-ProfileExists -Name $info.Standard -RG $info.RG
        if (-not $checkProfile) {
            $info.Status = "prepare-incomplete"; $info.Error = "New profile not found after prepare"
            Err "New profile not visible yet. Will retry commit anyway."
        } else {
