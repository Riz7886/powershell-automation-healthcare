...
$Ctx = Get-AzContext
if (-not $Ctx) {
    Write-Log "Not logged in. Run: Connect-AzAccount  then re-run this script." "ERROR"
    exit 1
}
...
# -------------------------------------------------------------------------------
Write-Log "Getting all subscriptions..." "SCAN"

$RawSubs = Get-AzSubscription -WarningAction SilentlyContinue
...
Write-Log "Found $($AllSubs.Count) subscription(s)" "SUCCESS"
...
for ($s = 0; $s -lt $AllSubs.Count; $s++) {
    ...
    $Switched = Set-AzContext -SubscriptionId $Sub.Id -WarningAction SilentlyContinue
    if (-not $Switched) {
    ...
    $CogSpeech = [System.Collections.ArrayList]::new()
    foreach ($Cog in $CogList) {
        if ($Cog.Kind -in @("SpeechServices", "CognitiveServices", "SpeechTranslation")) {
            $null = $CogSpeech.Add($Cog)
        }
    }

    if ($CogSpeech.Count -gt 0) {
        foreach ($Acct in $CogSpeech) {
        ...
        Write-Log "No Speech Cognitive Services found" "SUCCESS"
    }
    ...
    foreach ($App in $AppList) {
        ...
        $MatchedSettings = [System.Collections.ArrayList]::new()
        foreach ($S in $AppFull.SiteConfig.AppSettings) {
            $NameMatch  = ($S.Name  -like "*SPEECH*") -or ($S.Name  -like "*STT*")
            $ValueMatch = ($S.Value -like "*speechtotext/v3.0*") -or ($S.Value -like "*/v3.0*")
            if ($NameMatch -or $ValueMatch) {
                $null = $MatchedSettings.Add($S)
            }
        }

        if ($MatchedSettings.Count -eq 0) { continue }
        ...
        foreach ($S in $AppFull.SiteConfig.AppSettings) {
            $NewHash[$S.Name] = $S.Value
        }

        foreach ($S in $MatchedSettings) {
            $OldVal = $S.Value
        ...
        if ($NeedsUpdate -and (-not $ReportOnly)) {
            $FixResult = Set-AzWebApp -ResourceGroupName $App.ResourceGroup `
                                      -Name $App.Name `
                                      -AppSettings $NewHash
            ...
            if ($FixResult) {
                ...
                Write-Log "Fix failed for $($App.Name)" "ERROR"
                $null = $SubResult.Errors.Add("Fix failed: $($App.Name)")
            }
        }
    }

    Write-Log "Subscription done: Found=$($SubResult.Found.Count) Fixed=$($SubResult.Fixed.Count)" "SUCCESS"
...
foreach ($R in $AllResults) {
    ...
    if ($R.Found.Count -eq 0) {
        $Rows = "<tr><td colspan='5' style='text-align:center;color:#6b7280;padding:14px'>No Speech API v3.0 resources found</td></tr>"
    } else {
...
$Html += "<style>body{font-family:Arial,sans-serif;margin:0;background:#f8fafc;color:#1e293b}"
$Html += ".hdr{background:#991b1b;color:#fff;padding:26px 34px}.hdr h1{margin:0;font-size:22px}.hdr p{margin:6px 0 0;opacity:.85;font-size:12px}"
$Html += ".alert{background:#fef3c7;border-left:6px solid #d97706;padding:13px 34px;font-size:13px;color:#92400e;font-weight:bold}"
$Html += ".kpi{display:flex;gap:14px;padding:18px 34px;background:#fff;border-bottom:1px solid #e2e8f0}"
$Html += ".kpi-box{flex:1;border-radius:8px;padding:14px;text-align:center}.val{font-size:28px;font-weight:700}.lbl{font-size:12px;margin-top:4px}"
$Html += ".red{background:#fee2e2}.red .val{color:#991b1b}.green{background:#d1fae5}.green .val{color:#065f46}.blue{background:#dbeafe}.blue .val{color:#1e40af}.gray{background:#f1f5f9}.gray .val{color:#374151}"
$Html += ".sec{padding:22px 34px}.ftr{padding:14px 34px;text-align:center;color:#94a3b8;font-size:11px;border-top:1px solid #e2e8f0}"
$Html += "td{padding:7px 10px;border-bottom:1px solid #e2e8f0;vertical-align:top}tr:hover{background:#f1f5f9}</style></head><body>"
