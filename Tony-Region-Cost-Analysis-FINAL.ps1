...
if ($recommendedRegion) {
    $html += @"
        <div class="recommendation-box">
            <h3>YES RECOMMENDED ACTION: Migrate to $($recommendedRegion.RegionName)</h3>
...
"@
    ...
    $html += @"
        <div class="problem-box">
            <h3>WARNING WARNING: No viable regions found with sufficient quota!</h3>
...
foreach ($r in $regionAnalysis | Sort-Object -Property TotalMonthlyCost) {
    ...
    $quotaStatus = if ($r.CanSupportDatabricks) { 
        "<span class='has-quota'>YES YES</span>" 
    } else { 
        "<span class='no-quota'>NO NO</span>" 
    }
